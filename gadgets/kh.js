...
function getCssTransform() {
	...
	alert("您的浏览器不支持 CSS tranforms!");
	throw "您的浏览器不支持 CSS tranforms!";
}
getCssTransform();
...
function whenAllLoaded(gameDiv, popup, stickyNodes) {
	...
	jQuery('<button>开始!</button>').click(function () {
...
<p>控制: 按住 <b><select id="mouseb">\
<option value="0" selected="selected">左键选中</option>\
<option value="2">右键选中</option>\
<option value="-5">触摸</option>\
</select></b> 来控制这个球的滚动!</p>\
<div><label>背景音乐? \
<input id="bgmusicc" type="checkbox" checked="checked" /></label></div>\
<div style="text-align:right; color:gray;">\
<label>Katamari 颜色: <select id="khcolor">\
...
<option selected="selected" value="#7D26CD" style="background-color:#7D26CD;color:#7D26CD"> p \
 ...
 <label title="如果游戏运行缓慢将这个调低一点.">\
最大附着对象: <select id="maxAtt">\
<option>25</option>\
<option>50</option>\
<option selected="selected">75</option>\
<option>100</option>\
<option>9000</option></select></label><br />\
<label title="当一个对象被拾起时，要增长多少.">\
生长速度: <input id="vol_mult" type="text" size="6" value="1.0" />\
</label><br />\
<label title="更大的物体需要一个更大的块捡起来.">\
现实拾音器? <input id="checkv" type="checkbox" checked="checked" />\
</label></div>\
<p id="loadingp">加载!</p>';
