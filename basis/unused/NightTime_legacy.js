...
function CurrentModeStyle() {
	css = document.getElementById("css");
	if (css) theme = css.getAttribute("data-theme");
	if (darkModeFact == 1) DarkModeStyle();
	else LightModeStyle();
}
CurrentModeStyle();
function ToggleModeStyle() {
	darkModeFact = !darkModeFact;
	CurrentModeStyle();
}
//});
...
`<style id="light-root">
	...
	// } catch (e) {}
	try {
		if (lightDo && typeof(lightDo) == "function") lightDo();
		//用户可以定义一个名为lightDo的函数，自由决定要在浅色模式下想做的操作
	} catch (e) {}
}
